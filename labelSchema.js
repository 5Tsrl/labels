...
function getRegionalValue(record) {
  ...
  } else if (record.country_a === 'ITA' && _.isEqual(record.region, record.localadmin)) {
      // 5t se localadmin coincide con con region (= capoluogo di provincia), ometto la adminpart della provincia
      // console.log('capoluogo?', record.region, record.localadmin, _.isEqual(record.region, record.localadmin))
      return '';
